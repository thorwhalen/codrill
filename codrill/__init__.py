...
def callables_of_module(m):
    for func in filter(
        lambda func: getattr(func, "__module__", None) == m.__name__,
        filter(callable, map(lambda a: getattr(m, a), dir(m))),
    ):
        yield func


def snippets_of_funcs(
    funcs, max_code_lines=12, max_doc_lines=15, ignored_exceptions=(ValueError,)
):
    if ismodule(funcs):
...
class Exercises:
    def __init__(
        ...
        _gen = snippets_of_funcs(
            funcs, max_code_lines, max_doc_lines, ignored_exceptions
        )
